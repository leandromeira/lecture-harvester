...
def _extract_usage(provider: str, response):
    ...
    if provider_key == "openai":
        ...
        total_tokens = _as_int(getattr(usage, "total_tokens", input_tokens + output_tokens))
    ...
    if provider_key == "gemini":
        ...
        input_tokens = _as_int(getattr(usage_meta, "prompt_token_count", 0))
        output_tokens = _as_int(getattr(usage_meta, "candidates_token_count", 0))
        total_tokens = _as_int(getattr(usage_meta, "total_token_count", input_tokens + output_tokens))
        return input_tokens, output_tokens, total_tokens
